...
st.title('NFT sector growth in Ethereum Ecosystem')


# In[2]:


st.markdown('This part shows the basic NFT growth on **Ethereum** ecosystem. It is intended to provide an overview of the growth evolution of Ethereum NFT activity.')
...
count(distinct NFT_FROM_ADDRESS) as minters,
...
from ethereum.core.ez_nft_sales --y where trunc(x.block_timestamp,'day')=trunc(y.block_timestamp,'day')
group by 1
order by 1
),
  final_data as (
...
from ethereum.core.fact_event_logs x
  join ethereum.core.dim_labels y on x.contract_address=y.address
...
st.write("")
st.markdown('In this last charts can be seen the evolution of NFT market share accross Ethereum ecosystem.')
...
fig1.update_layout(
    title='Ethereum NFT events vs others over the past month',
    xaxis_tickfont_size=14,
